<template>
    <main class="home-page">
        <h1>Supplier List</h1>
        <p style="color:crimson">Manage Supplier</p>
            ...
            <tbody>
                <tr v-for="supplier in suppliers" :key="supplier.supplierName">
                    ...
                    <td v-if="appModel.roleId ===  2">
                        <button type="button" class="material-icons btn-sm bg-warning text-white" data-bs-toggle="modal" data-bs-target="#editModal" @click="editData(supplier.supplierName)">create</button>
                        &nbsp;&nbsp;
                        <button type="button" class="material-icons  btn-sm bg-danger text-white" disabled  data-toggle="tooltip" data-placement="top" title="No Delete Action">
                            block
                        </button>
                    ...
                    <td v-if="appModel.roleId ===  3">
                        <button type="button" class="material-icons btn-sm bg-warning text-white" data-bs-toggle="modal" data-bs-target="#editModal" @click="editData(supplier.supplierName)">create</button>
                        &nbsp;&nbsp;
                        <button type="button" class="material-icons  btn-sm bg-danger text-white" data-bs-toggle="modal" data-bs-target="#deleteModal" @click="getDeleteSupplier(supplier.supplierName)">delete_outline</button>
                    </td>
                </tr>
    ...
    methods:{
        addSupplier(){
            ...
            if(this.supplierName === ""){
                this.supplierDescriptionError = ""
                this.supplierNameError = "Supplier Name is required";
                return;
            }else if(this.supplierDescription === ""){
                this.supplierNameError = ""
                this.supplierDescriptionError = "SupplierDescription is required"
                return;
            }else{
            const formData = { 
            ...
            setTimeout(_ => {
            axios.post("http://localhost:8080/supplier/add", formData) .then(response => {
               
               if(response.status === 201){
                 this.isProccess = false;
                 alert("Supplier Successfully add");    
        ...
        editData(data) {
           this.supplierNameUpdate = data;
           axios.get(`http://localhost:8080/supplier/find/${data}`) .then(response => {
               
               if(response.status === 200){
                 // set values
                 this.supplierNameUpdate = response.data.supplierName;
                 this.supplierDescriptionUpdate = response.data.supplierDescription;                     
                }else{
        ...
        maintainSupplier(){
            ...
            if(this.supplierNameUpdate === ""){
                this.supplierDescriptionError = ""
                this.supplierNameError = "Supplier Name is required";
            }else if(this.supplierDescriptionUpdate === ""){
                this.supplierNameError = ""
                this.supplierDescriptionError = "SupplierDescription is required"
            }else{
            const formData = { 
                    supplierName: this.supplierNameUpdate, 
                    supplierDescription: this.supplierDescriptionUpdate, 
                    createdBy: this.appModel.email

                }
            this.isProccessMaintain = true; 
            setTimeout(_ => {
            axios.put(`http://localhost:8080/supplier/maintain/${formData.supplierName}`, formData) .then(response => {
        ...
        getDeleteSupplier(data){
           
            this.deleteSupplier = data;
            axios.get(`http://localhost:8080/supplier/find/${data}`) 
        },
        deleteSupplierById(){
           
            this.isProccessDelete = true; 
            setTimeout(_ => {
            axios.delete(`http://localhost:8080/supplier/delete/${this.deleteSupplier}`) .then(response => {
